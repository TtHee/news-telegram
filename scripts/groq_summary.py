...
分類規則（重要）：
- 此新聞的來源分類為 "{source_category}"，你應優先保留此分類
- 只有在新聞內容「明顯不屬於」來源分類時，才改成更合適的分類
- 科技公司的商業新聞（如 AI 公司併購、晶片出口管制）→ 仍歸 "ai"，不要改成 "finance"
- 台灣企業的國際佈局 → 仍歸 "stock_tw"，不要改成 "global"
- 美股個股的財報、併購 → 歸 "stock_us"，不要改成 "finance"

回覆格式（僅回覆 JSON，不要加任何其他文字）：
{{"title_zh": "繁體中文標題", "summary": "繁體中文摘要內容", "sentiment": "中性", "category": "{source_category}"}}"""""
...
def summarize(title: str, content: str = "", source_category: str = "global") -> dict:
    """
    回傳 {"title_zh": str, "summary": str, "sentiment": str, "category": str|None}
    失敗或未設定 API Key 時，回傳標題作為摘要。
    source_category: 來源的原始分類，供 AI 參考（優先保留）。
    """
    if not GROQ_API_KEY:
    ...
    prompt = SYSTEM_PROMPT.format(source_category=source_category)
